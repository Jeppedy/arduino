...
// NOPE!  13 is used for RF24 communication  
...
//  t2 = getDS18Temperature(ONEWIRE_PIN, true) ;
...
//  printf( "DS Temp=[" ); printFloat(t2,1); printf("]\n\r") ;
  ...
  // 2 
  strcat(outBuffer,",");
  sprintf(temp,"%04d",_t2); strcat(outBuffer,temp); // Append the Temp from DS18B20

  // 3 
  strcat(outBuffer,",");
  sprintf(temp,"%04d",_h); strcat(outBuffer,temp); // Append the Humidity from DHT11

  // 4 
  strcat(outBuffer,",");
  sprintf(temp,"%04d",_t); strcat(outBuffer,temp); // Append the Temp from DHT11
