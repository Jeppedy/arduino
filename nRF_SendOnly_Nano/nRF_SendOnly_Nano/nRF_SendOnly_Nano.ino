...
#define ONEWIRE_PIN1 7
#define ONEWIRE_PIN2 6
...
void loop(void)
{
  ...
  uint16_t _seq, _t, _t2, _m, _dummy;
    _seq = _t = _t2 = _m = _dummy = 9999;
  char temp[5];
  bool timeout=0;
...
//  float t = dht.readTemperature(true);
  float t  = getTemperature(ONEWIRE_PIN1, true) ;
  float t2 = getTemperature(ONEWIRE_PIN2, true) ;
...
//  printf( "DS Temp=[" ); printFloat(t2,1); printf("]\n\r") ;
  ...
  // 2 
  strcat(outBuffer,",");
  sprintf(temp,"%04d",_t);  // First OneWire DS18B20
  strcat(outBuffer,temp);

  // 3 
  strcat(outBuffer,",");
  sprintf(temp,"%04d",_t2);  // Second OneWire DS18B20
  strcat(outBuffer,temp);

  // 4 
  strcat(outBuffer,",");
  sprintf(temp,"%04d",_m);  // Motion
  strcat(outBuffer,temp);
