...
void NarcolepticClass::delay(unsigned long milliseconds) {
...
/* Narcoletic only handles up to 32768 seconds.  If you need more, handle it yourself */
void NarcolepticClass::longDelay(unsigned long myDelay)
{
  //printf("myDelay: [%lu]\n\r",myDelay) ;  Serial.flush() ;

  while (myDelay >= 32767) { Narcoleptic.delay(32767);    myDelay -= 32767; }

  //printf("And Finally [%lu]\n\r", myDelay) ;  Serial.flush() ;
  if( myDelay > 0 ) {Narcoleptic.delay(myDelay) ;}
}
